...
let createUser = Cb.Router.post(router, "/create_user", (req, res) => {
  let cFUNC = "createUser()"

  let createNewUser = (user: C.Rest.User.t): Js.Promise.t<Belt.Result.t<Js.Json.t, Js.Json.t>>  => {
    ...
    -> Js.Promise.then_((_) => {
      Js.Promise.resolve(Belt.Result.Ok(C.Rest.Reply.encode(~ok=true, ())))
    },_) 
    -> Js.Promise.catch((e) => {
  ...
  let verifyUserName = (user: C.Rest.User.t): Js.Promise.t<Belt.Result.t<Js.Json.t, Js.Json.t>> => {
    ...
    -> Js.Promise.then_((result: Cb.Pg.Query.result<CountResult.t>) => {
      if result.rows[0].count > 0 {
        ...
        -> Js.Promise.then_(message => {
          Js.Promise.resolve(Belt.Result.Error(C.Rest.Reply.encode(~ok=false, ~err="user_already_exists", ~message=message, ())))
        }, _)
      } else {
        createNewUser(user)
      }
  ...
  let verifyEmail = (user: C.Rest.User.t): Js.Promise.t<Belt.Result.t<Js.Json.t, Js.Json.t>> => {
    ...
    -> Js.Promise.then_((result: Cb.Pg.Query.result<CountResult.t>) => {
      if (result.rows[0].count > 0) {
        Cb.Translation.translateKey(~key="rest_create_user.email_already_exists", ())
         -> Js.Promise.then_((message) => {
          Js.Promise.resolve(Belt.Result.Error(C.Rest.Reply.encode(~ok=false, ~err="email_already_exists", ~message=message, ())))
        }, _)
          } else {
            verifyUserName(user)
          }
  ...
  | Ok(body) => 
    ...
    | Some(user) =>
      ...
      -> Js.Promise.catch((e) => {
        ...
        Js.Promise.resolve(Cb.Response.sendJson(res, C.Rest.Reply.encode(~ok=false, ~err="error", ())))
      },_)
    | None =>
        Cb.Response.status(res, 400) 
        Js.Promise.resolve(Cb.Response.sendJson(res, C.Rest.Reply.encode(~ok=false, ~err="could not json decode payload", ())))
    }
  | Error(reply) => 
      Cb.Response.status(res, 400) 
